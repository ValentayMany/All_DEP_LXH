...
// ============================================================
var API = window.location.origin; // ໃຊ້ same origin ອັດຕະໂນມັດ

// ============================================================
// STATE
// ============================================================
var CU = null, DOCS = [], DEPTS = [], TOKEN = "";
var CURRENT_KEY = null, CURRENT_DIR = null;
var FORM_KEY = null, FORM_DIR = null, EDIT_DOC_NUM = null;
...
var DEPT_ICONS = {
  "ບຸກຄະລາ": "👤", "ການເງິນ": "💰", "ບໍລິຫານ": "🏛️", "ວິຊາການ": "🔧",
  "ແຜນ": "📊", "ສາງ": "📦", "ຈັດຊື້": "🛒", "ຕະຫຼາດ": "📣",
  "ພັດທະນາ": "🚀", "ການແພດ": "🏥", "Partnership": "🤝", "ໄອທີ": "💻"
};
...
function getDeptIcon(name) {
  ...
  for (var k in DEPT_ICONS) { if (n.indexOf(k) >= 0) return DEPT_ICONS[k]; }
  return "📁";
}
...
function deptNameByKey(k) {
  var f = DEPTS.find(function (d) { return deptKey(d.name) === k; });
  return f ? f.name : "";
}
function esc(s) {
...
window.onload = function () {
  ...
  if (s && t) { CU = JSON.parse(s); TOKEN = t; showMain(); }
  else { if (!DEPTS.length) loadDepts(); }
};
...
async function doLogin() {
  ...
  hideErr("login-error");
  if (!u || !p) { showErr("login-error", "ກະລຸນາປ້ອນຊື່ຜູ້ໃຊ້ ແລະ ລະຫັດຜ່ານ"); return; }
  showLoading();
  var res = await api("POST", "/auth/login", { username: u, password: p });
  hideLoading();
  if (res && res.success) {
    CU = res.user; TOKEN = res.token;
    sessionStorage.setItem("lhms_u", JSON.stringify(CU));
    sessionStorage.setItem("lhms_t", TOKEN);
    showMain(); loadDepts();
  } else { showErr("login-error", res ? res.message : "ເກີດຂໍ້ຜິດພາດ"); }
}
...
async function doRegister() {
  ...
  hideErr("reg-error");
  if (!fn || !u || !p || !d) { showErr("reg-error", "ກະລຸນາຕື່ມຂໍ້ມູນໃຫ້ຄົບ"); return; }
  showLoading();
  var res = await api("POST", "/auth/register", { fullname: fn, username: u, password: p, department: d });
  hideLoading();
  if (res && res.success) { alert("ສ້າງບັນຊີສຳເລັດ!"); showLogin(); }
  else showErr("reg-error", res ? res.message : "ເກີດຂໍ້ຜິດພາດ");
}

function doLogout() {
  CU = null; DOCS = []; DEPTS = []; TOKEN = "";
...
function showMain() {
  ...
  document.getElementById("u-role").textContent = CU.role === "admin" ? "ຜູ້ດູແລລະບົບ" : "ຜູ້ໃຊ້ — " + CU.department;
...
function buildSidebarNav() {
  ...
  var visibleDepts;
  if (CU.role === "admin") { visibleDepts = DEPTS; }
  else {
    var allowedNames = DEPT_GROUP_MAP[cuDept] || [cuDept];
    visibleDepts = DEPTS.filter(function (d) { return allowedNames.indexOf(String(d.name || "").trim()) >= 0; });
    if (!visibleDepts.length && cuDept) visibleDepts = [{ id: "ud", name: cuDept }];
  }
  html += '<div class="nav-item" id="ni-dashboard" onclick="showDashboard()"><span style="font-size:16px">📊</span>Dashboard</div>';
  html += '<div class="sb-divider"></div><div class="nav-section-label">ພະແນກ</div>';
  visibleDepts.forEach(function (dept) {
    var k = deptKey(dept.name); var icon = getDeptIcon(dept.name);
    html += '<div><div class="nav-group-header" id="grp-' + k + '" onclick="toggleGroup(\'' + k + '\')">'
      + '<span class="ng-icon">' + icon + '</span><span style="flex:1">' + dept.name + '</span>'
      + '<span class="arrow">▶</span></div>'
      + '<div class="nav-sub" id="sub-' + k + '">'
      + '<div class="nav-sub-item" id="ni-' + k + '-in" onclick="showDocSec(\'' + k + '\',\'in\')"><span class="sub-icon">📥</span>ຂາເຂົ້າ</div>'
      + '<div class="nav-sub-item" id="ni-' + k + '-out" onclick="showDocSec(\'' + k + '\',\'out\')"><span class="sub-icon">📤</span>ຂາອອກ</div>'
      + '</div></div>';
  });
  html += '<div class="sb-divider"></div><div class="nav-item" id="ni-about" onclick="showAbout()"><span style="font-size:16px">ℹ️</span>About</div>';
  if (CU.role === "admin") {
    html += '<div class="sb-divider"></div><div class="nav-section-label">ລະບົບແອດມິນ</div>';
    html += '<div class="nav-item" id="ni-approvers" onclick="showApproversSec()"><span style="font-size:16px">✍️</span>ຈັດການຜູ້ອານຸມັດ</div>';
  }
  ...
  if (visibleDepts.length === 1) {
    ...
    setTimeout(function () {
      ...
      if (sub) sub.classList.add("open"); if (head) head.classList.add("open");
    }, 60);
  }
}

function toggleGroup(k) {
  var sub = document.getElementById("sub-" + k); var head = document.getElementById("grp-" + k);
  var wasOpen = sub.classList.contains("open");
  document.querySelectorAll(".nav-sub").forEach(function (s) { s.classList.remove("open"); });
  document.querySelectorAll(".nav-group-header").forEach(function (h) { h.classList.remove("open"); });
  if (!wasOpen) { sub.classList.add("open"); head.classList.add("open"); }
}
...
function setActiveSection(secEl, navId) {
  ...
  document.querySelectorAll(".nav-sub-item,.nav-item").forEach(function (n) { n.classList.remove("active"); });
  secEl.classList.add("active"); secEl.style.display = "flex";
  var ni = document.getElementById(navId); if (ni) ni.classList.add("active");
}
...
function showDashboard() {
  ...
  CURRENT_KEY = null; CURRENT_DIR = null; loadDashboard();
  closeMobileSB();
}
function showAbout() { setActiveSection(document.getElementById("sec-about"), "ni-about"); }

function showApproversSec() {
  setActiveSection(document.getElementById("sec-approvers"), "ni-approvers");
  CURRENT_KEY = null; CURRENT_DIR = null;
  loadApprovers();
  closeMobileSB();
}
...
function showDocSec(k, dir) {
  ...
  setActiveSection(document.getElementById("sec-" + k + "-" + dir), "ni-" + k + "-" + dir);
  CURRENT_KEY = k; CURRENT_DIR = dir;
...
async function loadDashboard() {
  var cuDept = String(CU.department || "").trim();
  var allowedNames = CU.role === "admin" ? DEPTS.map(function (d) { return d.name; }) : (DEPT_GROUP_MAP[cuDept] || [cuDept]);
  var container = document.getElementById("db-dept-cards");
  container.innerHTML = '<div style="color:var(--gray-400);font-size:13px;padding:20px 0">⏳ ກຳລັງໂຫລດ...</div>';
  showLoading();
  var params = CU.role === "admin" ? "role=admin" : "dept=" + encodeURIComponent(allowedNames.join(",")) + "&role=user";
  var [res, appRes] = await Promise.all([
    api("GET", "/documents?" + params),
    api("GET", "/approvers/active")
  ]);
  hideLoading();
  if (!res || !res.success) { container.innerHTML = '<div style="color:var(--red)">❌ Error</div>'; return; }

  var all = res.data || [];
  var visible = CU.role === "admin" ? all : all.filter(function (d) { return allowedNames.indexOf(String(d.department || "").trim()) >= 0; });
  var activeApprovers = appRes && appRes.success ? appRes.data || [] : [];

  document.getElementById("db-total").textContent = visible.length;
  document.getElementById("db-in").textContent = visible.filter(function (d) { return d.docType === "ຂາເຂົ້າ"; }).length;
  document.getElementById("db-out").textContent = visible.filter(function (d) { return d.docType === "ຂາອອກ"; }).length;
  ...
  visible.forEach(function (d) {
    ...
    byDept[dep].total++;
    if (d.docType === "ຂາເຂົ້າ") byDept[dep].in++; else byDept[dep].out++;
    var appBy = String(d.approvedBy || "").trim();
    if (appBy) byDept[dep].approvers[appBy] = (byDept[dep].approvers[appBy] || 0) + 1;
  });

  DEPT_APPROVER_DATA = byDept;

  var keys = Object.keys(byDept);
  if (!keys.length) { container.innerHTML = '<div style="color:var(--gray-400);font-size:13px;padding:20px 0">📭 ຍັງບໍ່ມີຂໍ້ມູນ</div>'; return; }

  container.innerHTML = keys.map(function (dep) {
    var s = byDept[dep]; var icon = getDeptIcon(dep);
    var hasApprovers = Object.values(s.approvers).some(function(v){ return v > 0; });
    return '<div class="db-dept-card" onclick="openDeptModal(\'' + dep.replace(/'/g, "\\'") + '\')">'
      + '<div class="db-dept-card-head"><div class="db-dept-icon">' + icon + '</div><span>' + esc(dep) + '</span></div>'
      + '<div class="db-dept-stats">'
      + '<div class="db-dept-stat"><div class="db-dept-stat-val">' + s.total + '</div><div class="db-dept-stat-lbl">ທັງໝົດ</div></div>'
      + '<div class="db-dept-stat"><div class="db-dept-stat-val clr-in">' + s.in + '</div><div class="db-dept-stat-lbl">ຂາເຂົ້າ</div></div>'
      + '<div class="db-dept-stat"><div class="db-dept-stat-val clr-out">' + s.out + '</div><div class="db-dept-stat-lbl">ຂາອອກ</div></div>'
      + '</div>'
      + (hasApprovers ? '<div class="db-dept-hint">👆 ກົດເບິ່ງຜູ້ອານຸມັດ</div>' : '')
      + '</div>';
  }).join("");
}
...
function openDeptModal(dep) {
  ...
  document.getElementById("dept-modal-sub").textContent = "ທັງໝົດ " + s.total + " ລາຍການ";
  document.getElementById("dept-modal-stats").innerHTML =
    '<div class="dms-item total"><div class="dms-val">' + s.total + '</div><div class="dms-lbl">ທັງໝົດ</div></div>'
    + '<div class="dms-item inn"><div class="dms-val">' + s.in + '</div><div class="dms-lbl">ຂາເຂົ້າ</div></div>'
    + '<div class="dms-item out"><div class="dms-val">' + s.out + '</div><div class="dms-lbl">ຂາອອກ</div></div>';
  var approverKeys = Object.keys(s.approvers);
  var listHtml = approverKeys.length === 0
    ? '<div class="dml-empty">ຍັງບໍ່ມີຂໍ້ມູນຜູ້ອານຸມັດ</div>'
    : '<div class="dml-title">✍️ ສະຫຼຸບການອານຸມັດ</div>'
      + approverKeys.map(function(name) {
          var count = s.approvers[name];
          var pct = s.total > 0 ? Math.round(count / s.total * 100) : 0;
          return '<div class="dml-row">'
            + '<div class="dml-info"><span class="dml-name">' + esc(name) + '</span>'
            + '<span class="dml-count">' + count + ' ລາຍການ</span></div>'
            + '<div class="dml-bar-wrap"><div class="dml-bar" style="width:' + pct + '%"></div></div>'
            + '</div>';
        }).join("");
...
function ensureDocSection(k, dir) {
  ...
  div.id = secId; div.className = "section";
  var deptName = deptNameByKey(k) || String(CU.department || "").trim();
  var dirLabel = dir === "in" ? "ຂາເຂົ້າ" : "ຂາອອກ";
  var dirIcon = dir === "in" ? "📥" : "📤";
  div.innerHTML =
    '<div class="sec-head">'
    + '<div class="sec-head-left"><h2>' + dirIcon + ' ' + deptName + ' — ' + dirLabel + '</h2>'
    + '<p>ລາຍການເອກະສານ' + dirLabel + 'ທັງໝົດ</p></div>'
    + '<div class="sec-head-right">'
    + '<select id="dept-filter-' + k + '-' + dir + '" class="dept-filter-sel" onchange="applyDeptFilter(\'' + k + '\',\'' + dir + '\')"></select>'
    + '<button class="btn-add" onclick="openForm(\'' + k + '\',\'' + dir + '\',\'' + deptName + '\')">+ ເພີ່ມ' + dirLabel + '</button>'
    + '</div></div>'
    + '<div class="stats-row" id="stats-' + k + '-' + dir + '">'
    + '<div class="stat-card"><div class="stat-icon total">📊</div><div><div class="stat-val" id="st-total-' + k + '-' + dir + '">0</div><div class="stat-lbl">ທັງໝົດ</div></div></div>'
    + '<div class="stat-card"><div class="stat-icon in">📥</div><div><div class="stat-val" id="st-in-' + k + '-' + dir + '">0</div><div class="stat-lbl">ຂາເຂົ້າ</div></div></div>'
    + '<div class="stat-card"><div class="stat-icon out">📤</div><div><div class="stat-val" id="st-out-' + k + '-' + dir + '">0</div><div class="stat-lbl">ຂາອອກ</div></div></div>'
    + '</div>'
    ...
    + '<tbody id="tb-' + k + '-' + dir + '"><tr><td colspan="11" class="empty-cell">⏳ ກຳລັງໂຫລດ...</td></tr></tbody>'
    + '</table></div>';
...
async function loadDocsBySection(k, dir) {
  if (!CU) return;
  if (!DEPTS || !DEPTS.length) { await loadDepts(); setTimeout(function () { loadDocsBySection(k, dir); }, 300); return; }
  var deptName = deptNameByKey(k) || String(CU.department || "").trim();
  var cuDept = String(CU.department || "").trim();
  var allowedNames = DEPT_GROUP_MAP[cuDept] || [cuDept];
  ...
  if (!tb) return;
  tb.innerHTML = '<tr><td colspan="11" class="empty-cell">⏳ ກຳລັງໂຫລດ...</td></tr>';
  // Fetch only this section's specific department (not the whole group)
  var fetchDepts = CU.role === "admin" ? DEPTS.map(function (d) { return d.name; }) : [deptName];
  var params = "role=" + CU.role + "&" + fetchDepts.map(function (d) { return "dept=" + encodeURIComponent(d); }).join("&");
...
function buildDeptFilter(k, dir) {
  var sel = document.getElementById("dept-filter-" + k + "-" + dir); if (!sel) return;
  var thisDeptName = deptNameByKey(k) || "";
  var cuDept = String(CU.department || "").trim();
  var relatedDepts;
  if (CU.role === "admin") { relatedDepts = DEPTS.map(function (d) { return d.name; }); }
  else {
    var g = DEPT_GROUP_MAP[thisDeptName] || [thisDeptName];
    var a = DEPT_GROUP_MAP[cuDept] || [cuDept];
    relatedDepts = g.filter(function (n) { return a.indexOf(n) >= 0; });
    if (!relatedDepts.length) relatedDepts = [thisDeptName];
  }
  var prev = sel.value; sel.innerHTML = "";
  var all = document.createElement("option"); all.value = ""; all.textContent = "📋 ທັງໝົດ"; sel.appendChild(all);
...
function applyDeptFilter(k, dir) {
  ...
  var groupOfSec = CU.role === "admin" ? DEPTS.map(function (d) { return d.name; }) : [thisDeptName];
  var allowedForUser = CU.role === "admin" ? DEPTS.map(function (d) { return d.name; }) : (DEPT_GROUP_MAP[cuDept] || [cuDept]);
  var validDepts = CU.role === "admin" ? groupOfSec : groupOfSec.filter(function (n) { return allowedForUser.indexOf(n) >= 0; });
  var base = DOCS.filter(function (d) {
    var deptOk = selectedDept ? String(d.department || "").trim() === selectedDept : validDepts.indexOf(String(d.department || "").trim()) >= 0;
    return deptOk && d.docType === docType;
  });
  var q = document.getElementById("search").value.toLowerCase();
  var result = q ? base.filter(function (d) { return Object.values(d).some(function (v) { return String(v).toLowerCase().indexOf(q) >= 0; }); }) : base;
...
function updateStats(k, dir, docs) {
  ...
  if (i) i.textContent = docs.filter(function (d) { return d.docType === "ຂາເຂົ້າ"; }).length;
  if (o) o.textContent = docs.filter(function (d) { return d.docType === "ຂາອອກ"; }).length;
}

function renderTableTo(tbId, docs, k, dir) {
  var tb = document.getElementById(tbId); if (!tb) return;
  if (k && dir) updateStats(k, dir, docs);
  if (!docs.length) { tb.innerHTML = '<tr><td colspan="11" class="empty-cell">📭 ຍັງບໍ່ມີຂໍ້ມູນ</td></tr>'; return; }
  tb.innerHTML = docs.map(function (d) {
    var editBtn = '<button class="btn-edit" onclick="openEditForm(\'' + esc(d.docNumber) + '\')" title="ແກ້ໄຂ">✏️</button>';
    var delBtn = CU.role === "admin" ? '<button class="btn-del" onclick="delDoc(\'' + esc(d.docNumber) + '\')" title="ລຶບ">🗑</button>' : "";
    var badge = d.docType === "ຂາເຂົ້າ" ? '<span class="badge badge-in">📥 ຂາເຂົ້າ</span>' : '<span class="badge badge-out">📤 ຂາອອກ</span>';
    return '<tr>'
      + '<td class="doc-num">' + esc(d.docNumber) + '</td>'
      + '<td>' + esc(d.docDate) + '</td><td>' + esc(d.docTime) + '</td>'
      + '<td>' + esc(d.subject) + '</td><td>' + esc(d.recipient) + '</td>'
      + '<td>' + esc(d.requesterDept) + '</td><td>' + esc(d.department) + '</td>'
      + '<td>' + esc(d.approvedBy) + '</td>'
      + '<td class="doc-det" title="' + esc(d.details) + '">' + esc(d.details) + '</td>'
      + '<td>' + badge + '</td><td><div class="table-actions">' + editBtn + delBtn + '</div></td></tr>';
  }).join("");
}
...
async function openForm(k, dir, forceDeptName) {
  FORM_KEY = k; FORM_DIR = dir; EDIT_DOC_NUM = null;
  var deptName = forceDeptName || deptNameByKey(k) || "";
  var dirLabel = dir === "in" ? "ຂາເຂົ້າ" : "ຂາອອກ";
  var now = new Date();
  document.getElementById("modal-title").textContent = (dir === "in" ? "📥 " : "📤 ") + "ເພີ່ມ" + dirLabel + " — " + deptName;
  ...
  var res = await api("GET", "/documents/next-number?dept=" + encodeURIComponent(deptName));
  document.getElementById("f-num").value = res && res.docNumber ? res.docNumber : "";
...
function openEditForm(docNum) {
  var d = DOCS.find(function (x) { return x.docNumber === docNum; }); if (!d) return;
  FORM_KEY = CURRENT_KEY; FORM_DIR = CURRENT_DIR; EDIT_DOC_NUM = docNum;
...
function closeModal() {
  ...
  if (btn) { btn.disabled = false; btn.textContent = "💾 ບັນທຶກ"; }
}

async function submitDoc() {
  var fields = [
    { id: "f-num" }, { id: "f-subj" }, { id: "f-recv" }, { id: "f-dept" }, { id: "f-type" }, { id: "f-approved" }
  ];
  var valid = true;
  ...
  var res;
  if (EDIT_DOC_NUM) {
    res = await api("PUT", "/documents/" + encodeURIComponent(EDIT_DOC_NUM), doc);
  } else {
...
document.addEventListener("keydown", function (e) {
  ...
  else if (!document.getElementById("reg-form").classList.contains("hidden")) doRegister();
});
...
async function loadApproversForForm(deptName, currentValue) {
  ...
  var res = await api("GET", "/approvers/active?dept=" + encodeURIComponent(deptName));
  ...
  if (currentValue && list.indexOf(currentValue) < 0) {
    var o = document.createElement("option");
    o.value = currentValue; o.textContent = currentValue + " (ປິດໃຊ້ງານ)";
    sel.appendChild(o);
  }
...
function renderApproversTable() {
  ...
  if (!container) return;
  if (!APPROVERS.length) {
    container.innerHTML = '<div style="color:var(--gray-400);font-size:13px;padding:20px 0">📭 ຍັງບໍ່ມີຂໍ້ມູນ</div>';
    return;
  }
  ...
  Object.keys(groups).sort().forEach(function (dept) {
    ...
    var activeCount = list.filter(function(a){ return a.is_active; }).length;
    html += '<div class="apr-dept-card">';
    html += '<div class="apr-dept-head">'
      + '<div class="apr-dept-title"><span class="apr-dept-icon">' + getDeptIcon(dept) + '</span>' + esc(dept) + '</div>'
      + '<span class="apr-dept-count">' + activeCount + '/' + list.length + ' Active</span>'
      + '</div>';
    html += '<div class="apr-list">';
    list.forEach(function (ap) {
      var isActive = ap.is_active;
      html += '<div class="apr-row' + (isActive ? '' : ' apr-row-inactive') + '">'
        + '<div class="apr-row-info">'
        + '<div class="apr-avatar">' + esc((ap.name || '?')[0]).toUpperCase() + '</div>'
        + '<span class="apr-name">' + esc(ap.name) + '</span>'
        + '</div>'
        + '<div class="apr-row-actions">'
        + '<span class="apr-badge ' + (isActive ? 'apr-badge-on' : 'apr-badge-off') + '">' + (isActive ? 'Active' : 'Inactive') + '</span>'
        + '<button class="apr-btn apr-btn-edit" onclick="openApproverEditForm(\'' + ap.id + '\')" title="ແກ້ໄຂ">✏️</button>'
        + '<button class="apr-btn ' + (isActive ? 'apr-btn-off' : 'apr-btn-on') + '" onclick="toggleApproverStatus(\'' + ap.id + '\',' + isActive + ')" title="' + (isActive ? 'ປິດ' : 'ເປີດ') + '">'
        + (isActive ? '🔒' : '🔓') + '</button>'
        + '</div>'
        + '</div>';
    });
...
function openApproverForm() {
  ...
  document.getElementById("modal-approver-title").textContent = "➕ ເພີ່ມຜູ້ອານຸມັດ";
...
function openApproverEditForm(id) {
  var ap = APPROVERS.find(function (x) { return x.id === id; });
  if (!ap) return;
  EDIT_APPROVER_ID = id;
  document.getElementById("modal-approver-title").textContent = "✏️ ແກ້ໄຂຜູ້ອານຸມັດ";
...
async function submitApprover() {
  ...
  var res;
  if (EDIT_APPROVER_ID) {
    res = await api("PUT", "/approvers/" + EDIT_APPROVER_ID, { name: name, department: dept });
  } else {
...
async function toggleApproverStatus(id, currentActive) {
  var confirmMsg = currentActive ? "ຕ້ອງການປິດໃຊ້ງານຜູ້ອານຸມັດຄົນນີ້ແທ້ບໍ?" : "ຕ້ອງການເປີດໃຊ້ງານຜູ້ອານຸມັດຄົນນີ້ແທ້ບໍ?";
